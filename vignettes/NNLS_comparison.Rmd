...
se_brain_spatial[["celltypeprops"]] <- 
  readRDS(file = "../vignettes/comparison/mousebrain/celltypeprops")
se_brain_spatial[["stereoscope"]] <- 
  readRDS(file = "../vignettes/comparison/mousebrain/stereoscope")
```
...
If we obtain results with cell2location, maybe we can summarize the results like this:
...
# Let's pretend that corMat2 holds the correlation scores for NNLS vs cell2location
corMat2 <- corMat

corMat[upper.tri(corMat, diag = F)] <- NA
corMat2[lower.tri(corMat2, diag = F)] <- NA
...
p1 <- ggplot(gg1, aes(CellType_NNLS, CellType_stereoscope, fill = correlation)) +
  geom_tile() +
  scale_fill_gradientn(colours = RColorBrewer::brewer.pal(n = 11, name = "RdBu") |> rev(), limits = c(-1, 1)) +
...
gg2 <- corMat2 |> 
  ...
  rownames_to_column(var = "CellType_NNLS") |> 
  as_tibble() |> 
  tidyr::pivot_longer(all_of(rownames(corMat)), names_to = "CellType_cell2location", 
                      values_to = "correlation") |> 
  na.omit()

p2 <- ggplot(gg2, aes(CellType_NNLS, CellType_cell2location, fill = correlation)) +
  geom_tile() +
  scale_fill_gradientn(colours = RColorBrewer::brewer.pal(n = 11, name = "RdBu") |> rev(), limits = c(-1, 1)) +
  scale_y_discrete(limits = rev) +
...
```{r}
DefaultAssay(se_kidney_spatial) <- "Spatial"
# se_kidney_spatial <- SCTransform(se_kidney_spatial, assay = "Spatial", variable.features.n = 1e4)
# se_kidney_singlecell <- SCTransform(se_kidney_singlecell, variable.features.n = 1e4)
se_kidney_spatial <- RunNNLS(object = se_kidney_spatial, 
                      singlecell_object = se_kidney_singlecell, 
                      singlecell_assay = "SCT", 
                      spatial_assay = "SCT",
                      groups = "celltype")
```
