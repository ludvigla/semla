...
Load Visium data with STUtility2 into a Seurat object. The following steps assumes that the mouse brain 10x Visium data is located in `./mousebrain/visium/`.
...
By computing the pair-wise correlation between cell types across spots, we can get an idea of which cell types often appear together in the same areas.

```{r fig.asp=1, fig.width=4}
cor_matrix <- FetchData(se_brain_spatial, selected_celltypes) |> cor()
diag(cor_matrix) <- NA
pheatmap::pheatmap(cor_matrix, 
                   cellwidth = 14, cellheight = 14, 
                   treeheight_col = 10, treeheight_row = 10, 
                   main = "Cell type spatial correlation\nwithin spots")
```
<br>
...
nmf_data <- nmf_data$w
rownames(nmf_data) <- rownames(se_brain_spatial@meta.data)
colnames(nmf_data) <- paste0("Factor_", 1:10)

cor_df_factor <- bind_cols(FetchData(se_brain_spatial, selected_celltypes), nmf_data) |> 
  cor() |>
  data.frame() |> 
  rownames_to_column(var = "Cell") |> 
  filter(Cell %in% selected_celltypes) |> 
  select( matches(c("Cell", paste0("Factor_", 1:10))) ) |> 
  tidyr::pivot_longer(cols = paste0("Factor_", 1:10), 
               names_to = "Factor", 
               values_to = "cor")
cor_df_factor$Factor <- factor(cor_df_factor$Factor, levels = paste0("Factor_", 1:10))

ggplot(cor_df_factor, aes(x=Factor, y=Cell, size=abs(cor), color=cor)) +
  geom_point() +
  labs(title="Cell type ~ NMF factor\ncorrelation within spots", x="Factor", y = "Cell type") +
  scale_colour_gradient2(high = ("darkorange"), mid = "white", low = ("steelblue"), midpoint = 0) +
  theme_bw() +
