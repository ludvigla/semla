...
The same Visium and reference scRNA-seq data sets were used as input for all cell type deconvolution methods tested. The code displayed in the [Cell type mapping with NNLS](https://ludvigla.github.io/semla/articles/cell_type_mapping_with_NNLS.html) vignette was used to produce the NNLS mapping results, while *stereoscope* and *cell2location* were run in python on high-performace computing servers using default parameter settings.
...
```{r fig.show='show', fig.width=13, fig.height=5.5, class.source = 'fold-hide'}
...
```{r fig.show='show', fig.width=18, fig.height=7, class.source = 'fold-hide'}
...
```{r fig.width=10, fig.height=5.5, class.source = 'fold-hide'}
gg_bar <- bind_rows(
