...
Let's demonstrate how it works with an example. If we want to find the spots that are neighbors to cluster 2, we set `column_name = "seurat_clusters"` and `column_labels = "2"`:
...
In our `meta.data` slot, we now have a new column called "nb_to_2", short for "neighbors to cluster 2". In the plot below, you can see that this column provides information about which spots are outside the border of cluster 2.

```{r fig.asp=1, fig.width=6}
MapLabels(se, crop_area = c(0.45, 0.6, 0.8, 0.93),
          column_name = "nb_to_2", drop_na = TRUE,
          image_use = "raw", pt_size = 3)
```

We can tweak the function to identify the inner border instead, i.e. the spots that are located at the "tumor edge". When we run `RegionNeighbors` with `mode="inner"`, we get a column called `inner_border_2` which tells us what spots are on the "tumor edge".
...
se <- RegionNeighbors(se, column_name = "seurat_clusters", 
                      column_labels = "2", mode = "inner_outer")
...
se <- RegionNeighbors(se, column_name = "seurat_clusters", column_key = "(all)nb_to_",
                      column_labels = "2", mode = "all_inner_outer")

MapLabels(se, crop_area = c(0.45, 0.6, 0.8, 0.93),
          column_name = "(all)nb_to_2",
          image_use = "raw", pt_size = 3, drop_na = TRUE)
```
...
```{r fig.asp=0.9}
border_markers <- FindMarkers(se, ident.1 = "nb_to_2", 
                              ident.2 = "2", group.by = "nb_to_2")
...
se_border_spots <- SubsetSTData(se, expression = nb_to_2 %in% c("2", "nb_to_2"))

# Violin plot
VlnPlot(se_border_spots, features = rownames(border_markers_up), group.by = "nb_to_2") &
