...
For the purpose of this analysis, we just need to define a region of interest in our tissue sections. We could just as well use the tissue morphology as a basis to define this region. From the plot above, we can see that some clusters are confined to distinct regions, for example cluster 10:

```{r fig.asp=1}
se$cluster_10 <- ifelse(se$seurat_clusters %in% "10", "10", NA)
MapLabels(se, column_name = "cluster_10", override_plot_dims = TRUE, 
...
Cluster 10 is concentrated to a region rich with cancer cells but there are also a few spots in other parts of the tissue. At this point we might only be interested in one of these regions in which case we can split cluster_10 into spatially disconnected compartments. For this purpose, we can use `DisconnectRegions`:

```{r fig.asp=1}
se <- DisconnectRegions(se, column_name = "seurat_clusters", selected_groups = "10")

MapLabels(se, column_name = "10_split", override_plot_dims = TRUE, 
          image_use = "raw", drop_na = TRUE, pt_size = 2) +
  plot_layout(guides = "collect") &
  theme(legend.position = "right") &
  guides(fill = guide_legend(override.aes = list(size = 3), ncol = 2))
...
Imagine that we are interested in looking at the expression of certain genes as a function of distance to a region of interest. We can compute distances from a region of interest (ROI) using `RadialDistances`. Here we'll use cluster 2 as our ROI for the computation.

```{r fig.asp=1}
se$cluster_2 <- ifelse(se$seurat_clusters %in% "2", "2", NA)
MapLabels(se, column_name = "cluster_2", override_plot_dims = TRUE, 
          image_use = "raw", drop_na = TRUE, pt_size = 2) +
  theme(legend.position = "right") &
...
```{r}
se <- RadialDistance(se, column_name = "seurat_clusters", selected_groups = "2")
```

We can illustrate the results by coloring the spots based on the radial distances from cluster "2":

```{r fig.asp=1}
MapFeatures(se, features = "r_dist_2", center_zero = TRUE, pt_size = 2, 
...
se$r_dist_2 <- (100/273)*se$r_dist_2
...
Here we can see that the expression of CRISP3 is high inside cluster "2" and declines rapidly at the border whereas IGLC2 show the opposite trend. We can also visualize these trends spatially:

```{r fig.asp=0.6}
MapFeatures(SubsetSTData(se, expression = r_dist_2 < 1e3), 
...
```{r fig.asp=1}
AnglePlot(se, column_name = "cluster_2", selected_group = "2", pt_size = 2,
...
Now that we have an idea about how the angles are defined, we can provide an interval of angles to narrow down the search area and calculate radial distances. The color scale in the plot below represents the radial distances from the tumor (cluster 2) center.

```{r fig.asp=1}
se <- RadialDistance(se, column_name = "seurat_clusters", selected_groups = "2", 
                     angles = c(200, 240))

MapFeatures(se, features = "r_dist_2", pt_size = 2)
```
...
MapLabels(se, column_name = "intervals_2", pt_size = 2, drop_na = TRUE,
          colors = RColorBrewer::brewer.pal(n = 6, name = "Spectral")) &
  guides(fill = guide_legend(override.aes = list(size = 5), ncol = 3))
```

Inside our `Seurat` object meta data slot, we now have three new columns: 'angle_2', 'r_dist_2' and 'intervals_2'. The 'intervals_2' columns tells us what slice each spot belongs to. With this data, we can modify our previous plot and split it by slice. If we look at the same genes as before (CRISP3 and IGLC2), we can see that the trends is quite similar in all 6 directions:
...
se[[]] |> 
  ...
  ggplot(aes(r_dist_2, value_centered, color = variable)) +
    geom_smooth(method = "gam", formula = y ~ s(x, bs = "cs")) +
    geom_vline(aes(xintercept = 0, color = "border"), linetype = "dashed") +
    facet_wrap(~ intervals_2, ncol = 3) +
    theme_minimal()
