...
saveRDS(sampled_indices, "../../manuscript/synthetic_data/sampled_indices")

# Export label matrix
saveRDS(label_matrix, "../../manuscript/synthetic_data/label_matrix")

# Export sampled counts
saveRDS(sampled_counts, "../../manuscript/synthetic_data/sampled_counts")

# Exsport synthetic count matrix
data.table::fwrite(mini_bulk_umis %>% t() %>% as.data.frame(), file = "../../manuscript/synthetic_data/mini_bulk_umis.tsv", quote = FALSE, sep = "\t", row.names = TRUE, col.names = TRUE)
```
...
dir.create("wynthetic_data")
data.table::fwrite(GetAssayData(se_allen, slot = "counts")[, barcodes$barcode] %>% t() %>% as.data.frame(), file = "synthetic_data/allen_brain_umis.tsv", quote = FALSE, sep = "\t", row.names = TRUE, col.names = TRUE)
writeLines(rownames(se_allen), con = "../../manuscript/synthetic_data/variable_genes.txt")
data.table::fwrite(se_allen@meta.data[barcodes$barcode, ] %>% select(subclass), file = "../../manuscript/synthetic_data/metadata.tsv", sep = "\t", quote = FALSE, row.names = TRUE, col.names = TRUE)
data.table::fwrite(GetAssayData(se_Visium_agg, slot = "counts") %>% t() %>% as.data.frame(), file = "../../manuscript/synthetic_data/Visium_synthetic_spots_umis.tsv", quote = FALSE, sep = "\t", row.names = TRUE, col.names = TRUE)
```
...
```{r eval=FALSE, include=FALSE}
mini_bulk_umis <- data.table::fread("../../manuscript/synthetic_data/mini_bulk_umis.tsv.gz") |> 
...
# Compare proportions
pred_all <- tibble(inf_props_nnls = GetAssayData(se_Visium_agg, slot = "data") |> t() |> as.numeric(),
...
```{r eval=FALSE}
stereoscope <- read.table("synthetic_data/stereoscope/stereoscope_proportions.csv", sep = ",", header = TRUE, row.names = 1, check.names = FALSE)
...
```{r eval=FALSE}
cell2location <- read.table("synthetic_data/cell2location/q05_cell_abundance_w_sf.csv", sep = ",", header = TRUE, row.names = 1, check.names = FALSE)
...
```{r include=FALSE, eval=FALSE}
run_time <- readRDS("synthetic_spots/results/run_time")
```
...
ggplot(pred_all_long |> filter(celltype %in% rownames(se_Visium_agg)[1:11]), 
       aes(exp_props, inf_props)) +
  geom_point(size = 0.5) +
...
ggplot(pred_all_long |> filter(celltype %in% rownames(se_Visium_agg)[11:22]), 
       aes(exp_props, inf_props)) +
  geom_point(size = 0.5) +
  facet_grid(method~celltype) +
  geom_smooth(method = lm) +
...
![6. Scatter plot showing expected vs inferred cell type porportions for 5 methods on 10,000 synthetic Visium spots. The blue line indicates a fitter trend line and the dashed red lines indicate the expected 1 to 1 relationship.](synthetic_spots/scatter_plots_comparison.jpg)
