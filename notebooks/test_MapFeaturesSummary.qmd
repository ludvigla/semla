...
MapFeaturesSummary <- function(
  ...
  # TODO: Add grid options (inherit from MapFeatures)
...
However, some things are not working for some reason, like `max_cutoff` (returns error)... Would we like this to cut off the summary plot as well? (probably no?)
...
As with all ggplots and patchworks we can also customize the theme.
...
Not sure why it looks like the grid is a bit off... The space between the spatial and summary plots is different for the two sections -- why??
