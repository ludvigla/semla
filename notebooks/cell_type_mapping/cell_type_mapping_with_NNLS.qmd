...
For this tutorial, we will use a single-cell dataset from the Allen Brain Atlas. You can check the code chunk below or follow [this](https://www.dropbox.com/s/cuowvm4vrf65pvq/allen_cortex.rds?dl=1) link to download it manually.
...
```{r}
coords <- GetStaffli(se_brain_spatial)@meta_data |> 
  filter(sampleID == 1)
xy <- coords |> 
...
expr_data <- FetchData(se_brain_spatial, cells = coords$barcode, 
                       vars = selected_celltypes) |> 
  as.matrix() |> 
...
gg <- bind_cols(xy, expr_data, opacity = apply(expr_data, 1, function(x) na.omit(x)))
gg_list <- lapply(selected_celltypes, function(x) {
  gg |> filter(!is.na(!! sym(x)))
}) |> 
  setNames(nm = selected_celltypes)

colors <- hsv(h = seq(0, 1, length.out = length(selected_celltypes) + 1), s = 1, v = 1)[-1]

p <- ggplot() +
  geom_point(data = gg_list[[selected_celltypes[1]]], 
             aes_string("x", "y", fill = paste0("`", selected_celltypes[1], "`")), 
             shape = 21, size = 2, 
             alpha = gg_list[[selected_celltypes[1]]]$opacity) +
  scale_fill_gradientn(colours = c("white", colors[1])) +
  scale_y_reverse() +
  coord_fixed() +
  theme_void()

for (i in 2:length(selected_celltypes)) {
  p <- p + new_scale_fill() +
    geom_point(data = gg_list[[selected_celltypes[i]]], 
             aes_string("x", "y", fill = paste0("`", selected_celltypes[i], "`")), 
             shape = 21, size = 2, 
             alpha = gg_list[[selected_celltypes[i]]]$opacity) +
    scale_fill_gradientn(colours = c("white", colors[i]))
}
