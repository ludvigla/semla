...
When working with multiple SRT datasets, it can be useful to align the tissue sections to have roughly the same orientation and size. In other words, we might want to register our H&E images to a reference image. When doing so, we want to apply rotations, reflections and translations. The issue with the default `magick` rotation function is that it creates a bounding box to hold the entire image, which makes the image bigger. In the plots above you cans see that the rotated image is 586x586 pixels in size compared to the 400x425 of the other images.
...
transf_res <- CoordAndImageTransform(im, xy_coords, angle = 45, xy_offset = c(0, 0))
