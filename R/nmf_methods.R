...
RunNNLS.Seurat <- function (
  ...
  if (verbose) inform(c("i" = "Fetching data from Seurat objects"))
...
.return_as <- function (
  ...
  # Return results as a DimReduc object or an Assay object
  if (return_as_dimred) {
    if (verbose) inform(c("v" = "Returning results as a 'DimReduc' object"))
    prop <- t(prop)
    ...
    if (verbose) inform(c("i" = glue("Returning results in a new 'Assay' named '{assay_name}'")))
    props_assay <- CreateAssayObject(data = as(prop, "dgCMatrix"))
    st_object[[assay_name]] <- props_assay
    if (verbose) inform(c("i" = glue("Setting default assay to '{assay_name}'")))
...
.prep_features <- function (
  ...
  # Filter features
  if (verbose) inform(c(">" = "  Filtering out features that are only present in one data set"))
  keep_features <- intersect(intersect(rownames(sc_object), rownames(st_object)), features)
  if (verbose) inform(c(">" = glue("  Kept {length(keep_features)} features for deconvolution")))
...
.get_w_matrix <- function (
  ...
  if (any(object < 0)) {
    ...
    if (verbose) inform(c("i" = "Rescaling data to ensure positive values"))
    object <- t(apply(object, 1, function(x) (x - min(x))/(max(x) - min(x))))
  }

  # Sample barcodes
  if (verbose) inform(c(">" = glue("  Downsampling scRNA-seq data to include a maximum of ",
  ...
  if (any(cells_n$n < minCells_per_celltype)) {
    ...
    if (verbose) inform(c(">" = cli::col_br_magenta(glue("  Cell type(s) {paste(too_small$group, collapse = ',')} ",
                           "have too few cells (<{minCells_per_celltype}) and will be excluded"))))
    barcodes <- barcodes |>
      filter(!group %in% too_small$group)
  }
  if (verbose) inform(c(">" = glue("  Kept {length(unique(barcodes$group))} cell types after filtering")))
  barcodes <- barcodes |>
    pull(barcode)
  new_groups <- setNames(groups, nm = colnames(singlecell_matrix))[barcodes]

  # Compute cell type expression profiles
  if (verbose) inform(c(">" = "  Calculating cell type expression profiles"))
